...
class TTTConfig:
    ...
    retouche_block_type: str = "cross"
    retouche_num_layers: int = 2
    retouche_low_rank_ratio: Optional[float] = 0.25
    retouche_alpha_init: float = 0.02
    retouche_alpha_shape: str = "per-channel"
    retouche_gate_lr_factor: float = 3.0
    retouche_label_smoothing: float = 0.15
    retouche_identity_guard: bool = True
    retouche_guard_tolerance: float = 0.005
...
if nn is not None:
    ...
    class RetoucheAdapter(nn.Module):
        def __init__(
            self,
            n_features: int,
            *,
            ...
            super().__init__()
            if n_features < 1:
                raise ValueError("RetoucheAdapter requires at least one feature")
            if int(num_layers) < 1:
                raise ValueError("--retouche-num-layers must be >= 1")

            block_type = str(block_type).lower()
            if block_type not in {"cross", "mlp"}:
                raise ValueError("--retouche-block-type must be one of: cross, mlp")

            alpha_shape = str(alpha_shape).lower()
            if alpha_shape not in {"per-channel", "global"}:
                raise ValueError("--retouche-alpha-shape must be one of: per-channel, global")
            ...
            alpha_size = self.n_features if alpha_shape == "per-channel" else 1
            self.alpha = nn.Parameter(torch.full((alpha_size,), float(alpha_init)))

            hidden = self._resolve_hidden_dim(self.n_features, low_rank_ratio)
            layers = []
            for _ in range(self.num_layers):
                if block_type == "cross":
                    layers.append(
                        nn.ModuleDict(
                            {
                                "down": nn.Linear(self.n_features, hidden, bias=False),
                                "up": nn.Linear(hidden, self.n_features, bias=True),
                            }
                        )
                    )
                else:
                    layers.append(
                        nn.ModuleDict(
                            {
                                "down": nn.Linear(self.n_features, hidden, bias=True),
                                "up": nn.Linear(hidden, self.n_features, bias=True),
                            }
                        )
                    )
        ...
        @staticmethod
        def _resolve_hidden_dim(n_features: int, low_rank_ratio: Optional[float]) -> int:
            if low_rank_ratio is None:
                return int(n_features)
            ratio = float(low_rank_ratio)
            if ratio <= 0:
                raise ValueError("--retouche-low-rank-ratio must be positive or None")
            return max(1, int(n_features * ratio))

        def reset_parameters(self) -> None:
            for layer in self.layers:
                nn.init.normal_(layer["down"].weight, mean=0.0, std=0.02)
                nn.init.normal_(layer["up"].weight, mean=0.0, std=0.02)
                if layer["down"].bias is not None:
                    nn.init.zeros_(layer["down"].bias)
                if layer["up"].bias is not None:
                    nn.init.zeros_(layer["up"].bias)
        ...
        def delta(self, x):
            ...
            for layer in self.layers:
                if self.block_type == "cross":
                    update = layer["up"](layer["down"](out))
                    out = x0 * update + out
                else:
                    out = out + layer["up"](torch.relu(layer["down"](out)))
            return out

        def forward(self, x):
            adapted = self.delta(x)
            alpha = self.alpha.to(dtype=x.dtype, device=x.device)
            if alpha.numel() == 1:
                return (1.0 - alpha) * x + alpha * adapted
            view_shape = (*([1] * (x.ndim - 1)), -1)
            alpha = alpha.view(view_shape)
            return (1.0 - alpha) * x + alpha * adapted


    class RetoucheModelWrapper(nn.Module):
        def __init__(self, base_model: nn.Module, adapter: RetoucheAdapter) -> None:
            super().__init__()
        ...
        def forward(self, *args, **kwargs):
            if "X" in kwargs:
                kwargs = dict(kwargs)
                kwargs["X"] = self.adapter(kwargs["X"])
            elif args:
                args = (self.adapter(args[0]), *args[1:])
            return self.base_model(*args, **kwargs)

        def clear_cache(self):
            clear_cache = getattr(self.base_model, "clear_cache", None)
            if callable(clear_cache):
                return clear_cache()
            return None

        def forward_with_cache(self, *args, **kwargs):
            if "X_train" in kwargs and kwargs["X_train"] is not None:
                kwargs = dict(kwargs)
                kwargs["X_train"] = self.adapter(kwargs["X_train"])
            if "X_test" in kwargs and kwargs["X_test"] is not None:
                kwargs = dict(kwargs)
                kwargs["X_test"] = self.adapter(kwargs["X_test"])
            return self.base_model.forward_with_cache(*args, **kwargs)


else:
    RetoucheAdapter = None
    RetoucheModelWrapper = None
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if str(args.retouche_block_type) not in {"cross", "mlp"}:
        raise ValueError("--retouche-block-type must be one of: cross, mlp")
    if int(args.retouche_num_layers) < 1:
        raise ValueError("--retouche-num-layers must be >= 1")
    if args.retouche_low_rank_ratio is not None and float(args.retouche_low_rank_ratio) <= 0:
        raise ValueError("--retouche-low-rank-ratio must be positive or None")
    if str(args.retouche_alpha_shape) not in {"per-channel", "global"}:
        raise ValueError("--retouche-alpha-shape must be one of: per-channel, global")
    if float(args.retouche_gate_lr_factor) <= 0:
        raise ValueError("--retouche-gate-lr-factor must be positive")
    if not 0.0 <= float(args.retouche_label_smoothing) < 1.0:
        raise ValueError("--retouche-label-smoothing must be in [0, 1)")
    if float(args.retouche_guard_tolerance) < 0:
        raise ValueError("--retouche-guard-tolerance must be >= 0")
    ...
    return TTTConfig(
        ...
        retouche_block_type=str(args.retouche_block_type),
        retouche_num_layers=int(args.retouche_num_layers),
        retouche_low_rank_ratio=(
            float(args.retouche_low_rank_ratio) if args.retouche_low_rank_ratio is not None else None
        ),
        retouche_alpha_init=float(args.retouche_alpha_init),
        retouche_alpha_shape=str(args.retouche_alpha_shape),
        retouche_gate_lr_factor=float(args.retouche_gate_lr_factor),
        retouche_label_smoothing=float(args.retouche_label_smoothing),
        retouche_identity_guard=bool(args.retouche_identity_guard),
        retouche_guard_tolerance=float(args.retouche_guard_tolerance),
    )
...
def _get_ttt_base_model(classifier):
    ...
    if RetoucheModelWrapper is not None and isinstance(model, RetoucheModelWrapper):
...
def _build_retouche_adapter(n_features: int, config: TTTConfig, device):
    if RetoucheAdapter is None:
        raise RuntimeError("PyTorch is required for Retouche adapter TTT")
    adapter = RetoucheAdapter(
        int(n_features),
        block_type=config.retouche_block_type,
        num_layers=config.retouche_num_layers,
        low_rank_ratio=config.retouche_low_rank_ratio,
        alpha_init=config.retouche_alpha_init,
        alpha_shape=config.retouche_alpha_shape,
    )
    return adapter.to(device)


def _retouche_optimizer_param_groups(adapter, config: TTTConfig) -> List[dict]:
    matrix_params = []
    no_decay_params = []
    gate_params = []
    ...
    groups: List[dict] = []
    if matrix_params:
        groups.append({"params": matrix_params, "lr": config.lr, "weight_decay": config.weight_decay})
    if no_decay_params:
        groups.append({"params": no_decay_params, "lr": config.lr, "weight_decay": 0.0})
    if gate_params:
        groups.append(
            {
                "params": gate_params,
                "lr": config.lr * config.retouche_gate_lr_factor,
                "weight_decay": 0.0,
            }
        )
    return groups
...
def _retouche_trainable_params(adapter) -> List[Any]:
    return [param for param in adapter.parameters() if param.requires_grad]


def _attach_retouche_adapter(classifier, adapter) -> None:
    base_model = _get_ttt_base_model(classifier)
    if isinstance(base_model, RetoucheModelWrapper):
        base_model.adapter = adapter
        return
    classifier._retouche_base_model_ = base_model
    classifier._retouche_adapter_ = adapter
    classifier.model_ = RetoucheModelWrapper(base_model, adapter)
    classifier.model_.to(classifier.device_)


def _detach_retouche_adapter(classifier) -> None:
    model = getattr(classifier, "model_", None)
    if RetoucheModelWrapper is not None and isinstance(model, RetoucheModelWrapper):
        classifier.model_ = model.base_model
    elif hasattr(classifier, "_retouche_base_model_"):
        classifier.model_ = classifier._retouche_base_model_


def _fit_preserving_model_weights_with_optional_adapter(classifier, X, y, adapter=None) -> None:
    if adapter is None:
        _detach_retouche_adapter(classifier)
    else:
        _attach_retouche_adapter(classifier, adapter)
    _fit_preserving_model_weights(classifier, X, y)


def _retouche_model_active(classifier) -> bool:
    model = getattr(classifier, "model_", None)
    return RetoucheModelWrapper is not None and isinstance(model, RetoucheModelWrapper)


def _restore_ttt_training_fit_state(classifier, X_train, y_train, adapter=None) -> None:
    if adapter is None:
        _detach_retouche_adapter(classifier)
        _fit_preserving_model_weights(classifier, X_train, y_train)
    else:
        _fit_preserving_model_weights_with_optional_adapter(classifier, X_train, y_train, adapter=adapter)
...
def _evaluate_ttt_validation_metrics(
    ...
    *,
    ...
    original_model = getattr(classifier, "model_", None)
    ...
    try:
        ...
        _fit_preserving_model_weights_with_optional_adapter(classifier, X_context, y_context, adapter=adapter)
        ...
        if original_model is not None:
            classifier.model_ = original_model
...
def _save_ttt_model_ckpt(
    ...
    *,
    ...
    if adapter is None:
        adapter = getattr(classifier, "_retouche_adapter_", None)
    if adapter is None:
        raise RuntimeError("Retouche checkpoint save requested before adapter is available")
    ...
    checkpoint = {
        ...
        "adapter_config": {
            "n_features": int(getattr(adapter, "n_features", 0)),
            "block_type": str(config.retouche_block_type),
            "num_layers": int(config.retouche_num_layers),
            "low_rank_ratio": config.retouche_low_rank_ratio,
            "alpha_init": float(config.retouche_alpha_init),
            "alpha_shape": str(config.retouche_alpha_shape),
        },
        "ttt_metadata": {
            "model_name": str(model_name),
            "dataset_name": str(dataset_name),
            "step": int(step_idx),
            "checkpoint_type": "retouche_adapter",
        },
    }
    ...
    *,
    ...
    if args.ttt_enabled:
        eval_estimator_label = "_".join(
            [
                "retouche",
                f"block{args.retouche_block_type}",
                f"layers{args.retouche_num_layers}",
                f"alpha{args.retouche_alpha_init}",
                f"guard{_format_path_value(args.retouche_identity_guard)}",
...
def run_ttt_epoch_chunk_update(
    ...
    *,
    ...
    base_model = _get_ttt_base_model(classifier)
    _configure_ttt_trainable_params(classifier, config)
    adapter = _build_retouche_adapter(int(X_encoded.shape[1]), config, classifier.device_)
    trainable_params = _retouche_trainable_params(adapter)
    if not trainable_params:
        return TTTUpdateResult(
            ...
            steps=0,
            update_seconds=time.time() - update_start,
            reason="No trainable Retouche adapter parameters selected for TTT",
            epochs=0,
            chunks_per_epoch=chunks_per_epoch,
        )

    optimizer = torch.optim.AdamW(_retouche_optimizer_param_groups(adapter, config))
    ...
    try:
        ...
        if config.early_stopping and X_val is not None and y_val is not None and len(y_val) > 0:
            ...
            _restore_ttt_training_fit_state(classifier, X_train, y_train, adapter=None)
            ...
            for batch in iter_epoch_meta_batches(
                ...
                for start_idx in range(0, total_views, effective_micro_batch_size):
                    ...
                    with amp_ctx_factory():
                        logits = forward_model(adapter(X_batch), y_train_batch.float())
                        n_classes = int(y_train_batch.max().item()) + 1
                        logits_used = logits[..., :n_classes].reshape(-1, n_classes)
                        loss = F.cross_entropy(
                            logits_used,
                            y_query_batch.long().reshape(-1),
                            label_smoothing=float(config.retouche_label_smoothing),
                        )
                ...
                if _should_save_ttt_ckpt_step(config, update_steps):
                    ckpt_path = _save_ttt_model_ckpt(
                        classifier,
                        config,
                        model_name,
                        dataset_name,
        ...
        guard_enabled = bool(config.retouche_identity_guard)
        guard_passed = True
        guard_reason = None
        if guard_enabled and baseline_metric is not None and best_metric is not None:
            guard_passed = _ttt_metric_improved(best_metric, baseline_metric, config.retouche_guard_tolerance)
            if not guard_passed:
                guard_reason = (
                    f"Retouche identity guard bypassed adapter: best_{config.eval_metric}="
                    f"{best_metric:.6f} baseline_{config.eval_metric}={baseline_metric:.6f} "
                    f"tolerance={config.retouche_guard_tolerance}"
                )
        elif guard_enabled and (X_val is None or y_val is None or len(y_val) == 0):
            guard_reason = "Retouche identity guard skipped because no validation set was available"

        adapter.eval()
        if guard_passed:
            _attach_retouche_adapter(classifier, adapter)
            classifier._retouche_use_adapter_ = True
        else:
            _detach_retouche_adapter(classifier)
            classifier._retouche_use_adapter_ = False
        ...
        classifier.model_kv_cache_ = None
        _get_ttt_base_model(classifier).eval()

    return TTTUpdateResult(
        applied=bool(getattr(classifier, "_retouche_use_adapter_", True)),
        loss=last_loss,
        steps=update_steps,
        update_seconds=time.time() - update_start,
        reason=guard_reason,
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                try:
                    ...
                    if ttt_applied:
                        final_adapter = getattr(classifier, "_retouche_adapter_", None)
                        _fit_preserving_model_weights_with_optional_adapter(
                            classifier,
                            X_train,
                            y_train,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ttt-lr", type=float, default=5e-3)
    ...
    parser.add_argument("--ttt-grad-clip", type=float, default=2.0)
    ...
    parser.add_argument("--ttt-weight-decay", type=float, default=3e-3)
    parser.add_argument(
        "--ttt-epochs",
        "--ttt-steps",
        dest="ttt_epochs",
    ...
    parser.add_argument("--ttt-patience", type=int, default=10)
    ...
    parser.add_argument("--retouche-block-type", choices=["cross", "mlp"], default="cross")
    parser.add_argument("--retouche-num-layers", type=int, default=2)
    parser.add_argument("--retouche-low-rank-ratio", type=parse_optional_float, default=0.25)
    parser.add_argument("--retouche-alpha-init", type=float, default=0.02)
    parser.add_argument("--retouche-alpha-shape", choices=["per-channel", "global"], default="per-channel")
    parser.add_argument("--retouche-gate-lr-factor", type=float, default=3.0)
    parser.add_argument("--retouche-label-smoothing", type=float, default=0.15)
    parser.add_argument("--retouche-identity-guard", type=parse_bool, default=True)
    parser.add_argument("--retouche-guard-tolerance", type=float, default=0.005)
    parser.add_argument(
        "--ttt-save-ckpt",
        type=parse_bool,
        default=False,
        help="Whether to save intermediate Retouche adapter checkpoints during the TTT update path.",
    )
    parser.add_argument(
        "--ttt-save-ckpt-every",
        type=int,
