...
class ResultRow:
    ...
    ttt_gate_decision: Optional[str] = None
    ttt_gate_reason: Optional[str] = None
    ttt_gate_confidence_mean: Optional[float] = None
    ttt_gate_confidence_p10: Optional[float] = None
    ttt_gate_entropy_mean: Optional[float] = None
    ttt_gate_entropy_norm_mean: Optional[float] = None
    ttt_gate_stability_mean: Optional[float] = None
    ttt_gate_probe_seconds: float = 0.0
...
class TTTConfig:
    ...
    risk_gate_policy: str = "uncertain"
    risk_confidence_threshold: float = 0.70
    risk_entropy_threshold: float = 0.45
    risk_stability_threshold: float = 0.80
    risk_gate_probe_n_estimators: int = 8
...
class RiskGateResult:
    ...
    entropy_norm_mean: Optional[float]
...
class ModelSummaryRow:
    ...
    ttt_gate_apply_uncertain_count: int
    ttt_gate_skip_low_risk_count: int
    ttt_gate_probe_failed_run_ttt_count: int
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ttt_gate={row.ttt_gate_decision} "
            f"ttt_gate_conf={format_optional_float(row.ttt_gate_confidence_mean)} "
            f"ttt_gate_entropy_norm={format_optional_float(row.ttt_gate_entropy_norm_mean)} "
            f"ttt_gate_stability={format_optional_float(row.ttt_gate_stability_mean)}"
        )
    if row.status == "skip":
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if float(args.ttt_risk_confidence_threshold) < 0 or float(args.ttt_risk_confidence_threshold) > 1:
        raise ValueError("--ttt-risk-confidence-threshold must be in [0, 1]")
    if float(args.ttt_risk_entropy_threshold) < 0 or float(args.ttt_risk_entropy_threshold) > 1:
        raise ValueError("--ttt-risk-entropy-threshold must be in [0, 1]")
    if float(args.ttt_risk_stability_threshold) < 0 or float(args.ttt_risk_stability_threshold) > 1:
        raise ValueError("--ttt-risk-stability-threshold must be in [0, 1]")
    if int(args.ttt_risk_probe_n_estimators) < 1:
        raise ValueError("--ttt-risk-probe-n-estimators must be >= 1")
    ...
    return TTTConfig(
        ...
        risk_gate_enabled=bool(args.ttt_risk_gate),
        risk_gate_policy="uncertain",
        risk_confidence_threshold=float(args.ttt_risk_confidence_threshold),
        risk_entropy_threshold=float(args.ttt_risk_entropy_threshold),
        risk_stability_threshold=float(args.ttt_risk_stability_threshold),
        risk_gate_probe_n_estimators=int(args.ttt_risk_probe_n_estimators),
    )
...
def _softmax_np(logits: Any, *, axis: int = -1, temperature: float = 1.0) -> Any:
...
def _predict_per_ensemble_proba(classifier, X) -> Any:
    ...
    if validate_data is not None:
        X = validate_data(classifier, X, reset=False, dtype=None, skip_check_array=True)
    ...
    X_encoded = classifier.X_encoder_.transform(X)
    outputs = []
    data = classifier.ensemble_generator_.transform(X_encoded, mode="both", feature_mask=feature_mask)
    for norm_method, (Xs, ys) in data.items():
        if feature_mask is None:
            feature_shuffles = classifier.ensemble_generator_.feature_shuffles_[norm_method]
        else:
            feature_shuffles = classifier.ensemble_generator_.masked_feature_shuffles_[norm_method]
        outputs.append(classifier._batch_forward(Xs, ys, feature_shuffles))
    raw_outputs = np.concatenate(outputs, axis=0)

    class_shuffles = []
    for shuffles in classifier.ensemble_generator_.class_shuffles_.values():
        class_shuffles.extend(shuffles)
    n_estimators = min(len(class_shuffles), int(raw_outputs.shape[0]))
    if n_estimators <= 0:
        raise RuntimeError("Risk gate probe could not find ensemble members")

    per_member = []
    for idx in range(n_estimators):
        out = raw_outputs[idx][..., class_shuffles[idx]]
        if classifier.average_logits:
            out = _softmax_np(out, axis=-1, temperature=classifier.softmax_temperature)
        else:
            out = np.asarray(out, dtype=np.float64)
        out = out / np.clip(out.sum(axis=1, keepdims=True), 1e-12, None)
        per_member.append(out)

    return np.stack(per_member, axis=0)


def _compute_ensemble_stability(per_member_proba: Any, mean_proba: Any) -> Optional[float]:
    member_pred = np.argmax(np.asarray(per_member_proba), axis=2)
    if member_pred.ndim != 2 or member_pred.shape[0] == 0 or member_pred.shape[1] == 0:
        return None
    n_classes = int(np.asarray(mean_proba).shape[1])
    majority_pred = np.asarray(
        [
            np.bincount(member_pred[:, sample_idx], minlength=n_classes).argmax()
            for sample_idx in range(member_pred.shape[1])
        ]
    )
    agreement = member_pred == majority_pred.reshape(1, -1)
    return float(np.mean(agreement))


def _run_risk_aware_ttt_gate(
    classifier,
    X_train,
    y_train,
    X_test,
    ...
    *,
    ...
    try:
        if original_n_estimators is not None:
            classifier.n_estimators = int(config.risk_gate_probe_n_estimators)
        classifier.fit(X_train, y_train)
        mean_proba = np.asarray(classifier.predict_proba(X_test), dtype=np.float64)
        if mean_proba.ndim != 2 or mean_proba.shape[0] == 0 or mean_proba.shape[1] == 0:
            raise RuntimeError(f"invalid baseline probability shape: {mean_proba.shape}")

        proba_clipped = np.clip(mean_proba, 1e-12, 1.0)
        confidence = np.max(proba_clipped, axis=1)
        entropy = -np.sum(proba_clipped * np.log(proba_clipped), axis=1)
        denom = np.log(max(2, int(mean_proba.shape[1])))
        entropy_norm = entropy / denom

        per_member_proba = _predict_per_ensemble_proba(classifier, X_test)
        stability_mean = _compute_ensemble_stability(per_member_proba, mean_proba)

        confidence_mean = float(np.mean(confidence))
        confidence_p10 = float(np.percentile(confidence, 10))
        entropy_mean = float(np.mean(entropy))
        entropy_norm_mean = float(np.mean(entropy_norm))
        risk_reasons = []
        if confidence_mean < config.risk_confidence_threshold:
            risk_reasons.append(
                f"confidence_mean={confidence_mean:.6f}<threshold={config.risk_confidence_threshold:.6f}"
            )
        if entropy_norm_mean > config.risk_entropy_threshold:
            risk_reasons.append(
                f"entropy_norm_mean={entropy_norm_mean:.6f}>threshold={config.risk_entropy_threshold:.6f}"
            )
        if stability_mean is None:
            risk_reasons.append("stability_mean unavailable")
        elif stability_mean < config.risk_stability_threshold:
            risk_reasons.append(
                f"stability_mean={stability_mean:.6f}<threshold={config.risk_stability_threshold:.6f}"
            )

        if risk_reasons:
            decision = "apply_uncertain"
            reason = "uncertain baseline; " + "; ".join(risk_reasons)
        else:
            decision = "skip_low_risk"
            reason = (
                "low-risk baseline; "
                f"confidence_mean={confidence_mean:.6f}>={config.risk_confidence_threshold:.6f}; "
                f"entropy_norm_mean={entropy_norm_mean:.6f}<={config.risk_entropy_threshold:.6f}; "
                f"stability_mean={format_optional_float(stability_mean)}"
                f">={config.risk_stability_threshold:.6f}"
            )

        print(
            f"[ttt-risk-gate] model={model_name} dataset={dataset_name} decision={decision} "
            f"confidence_mean={confidence_mean:.6f} confidence_p10={confidence_p10:.6f} "
            f"entropy_norm_mean={entropy_norm_mean:.6f} "
            f"stability_mean={format_optional_float(stability_mean)}",
            flush=True,
        )
        ...
        reason = f"risk gate probe failed; run TTT fallback: {format_exception_for_csv(exc)}"
        print(
            f"[ttt-risk-gate] model={model_name} dataset={dataset_name} decision=probe_failed_run_ttt "
            f"reason={reason}",
            flush=True,
        )
        return RiskGateResult(
            decision="probe_failed_run_ttt",
            reason=reason,
...
def build_auto_out_dir(
    ...
    *,
    ...
    if args.ttt_enabled:
        ...
        if args.ttt_risk_gate:
            eval_parts.append(
                "riskgate_uncertain"
                f"_conf{args.ttt_risk_confidence_threshold}"
                f"_ent{args.ttt_risk_entropy_threshold}"
                f"_stab{args.ttt_risk_stability_threshold}"
            )
        eval_estimator_label = "_".join(eval_parts)
    else:
...
def build_model_summary_row(
    ...
    if len(ok_df) and "ttt_gate_decision" in ok_df.columns:
        gate_decisions = ok_df["ttt_gate_decision"].fillna("").astype(str)
        gate_apply_count = int(gate_decisions.eq("apply_uncertain").sum())
        gate_skip_low_risk_count = int(gate_decisions.eq("skip_low_risk").sum())
        gate_probe_failed_count = int(gate_decisions.eq("probe_failed_run_ttt").sum())
    else:
        gate_apply_count = 0
        gate_skip_low_risk_count = 0
        gate_probe_failed_count = 0
    ...
    if error is not None:
        return ModelSummaryRow(
            ...
            ttt_gate_apply_uncertain_count=0,
            ttt_gate_skip_low_risk_count=0,
            ttt_gate_probe_failed_run_ttt_count=0,
    ...
    return ModelSummaryRow(
        ...
        ttt_gate_apply_uncertain_count=gate_apply_count,
        ttt_gate_skip_low_risk_count=gate_skip_low_risk_count,
        ttt_gate_probe_failed_run_ttt_count=gate_probe_failed_count,
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ...
        ttt_gate_decision = None
        ttt_gate_reason = None
        ttt_gate_confidence_mean = None
        ttt_gate_confidence_p10 = None
        ttt_gate_entropy_mean = None
        ttt_gate_entropy_norm_mean = None
        ttt_gate_stability_mean = None
        ttt_gate_probe_seconds = 0.0
        n_train_b = 0
        n_holdout_c = 0
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                should_run_ttt = True
                if ttt_config.risk_gate_enabled:
                    gate_result = _run_risk_aware_ttt_gate(
                        classifier,
                        X_train,
                        y_train,
                        X_test,
                    ...
                    ttt_gate_decision = gate_result.decision
                    ttt_gate_reason = gate_result.reason
                    ttt_gate_confidence_mean = gate_result.confidence_mean
                    ttt_gate_confidence_p10 = gate_result.confidence_p10
                    ttt_gate_entropy_mean = gate_result.entropy_mean
                    ttt_gate_entropy_norm_mean = gate_result.entropy_norm_mean
                    ttt_gate_stability_mean = gate_result.stability_mean
                    ttt_gate_probe_seconds = float(gate_result.probe_seconds)
                    ttt_split_reason = append_ttt_reason(
                        ttt_split_reason,
                        f"risk_gate={ttt_gate_decision}: {ttt_gate_reason}",
                    )
                    should_run_ttt = gate_result.decision != "skip_low_risk"
        ...
        return ResultRow(
            ...
            ttt_gate_decision=ttt_gate_decision,
            ttt_gate_reason=ttt_gate_reason,
            ttt_gate_confidence_mean=ttt_gate_confidence_mean,
            ttt_gate_confidence_p10=ttt_gate_confidence_p10,
            ttt_gate_entropy_mean=ttt_gate_entropy_mean,
            ttt_gate_entropy_norm_mean=ttt_gate_entropy_norm_mean,
            ttt_gate_stability_mean=ttt_gate_stability_mean,
            ttt_gate_probe_seconds=ttt_gate_probe_seconds,
        )
    except Exception as exc:
        return ResultRow(
            dataset_name=dataset_dir.name,
...
def write_summary(
    ...
    lines = [
        ...
        f"ttt_gate_apply_uncertain_count: {gate_apply_count}",
        f"ttt_gate_skip_low_risk_count: {gate_skip_low_risk_count}",
        f"ttt_gate_probe_failed_run_ttt_count: {gate_probe_failed_count}",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "risk-aware gated epoch-shuffled chunk TTT and AMD/ROCm multi-GPU workers."
        )
    )
    ...
    parser.add_argument(
        "--out-dir",
        default="result/compare/Gate_ttt_ensemble32_small_lt2000",
        help=(
            ...
            "estimator counts, risk-gate settings, and random seed."
        ),
    )
    ...
    parser.add_argument(
        ...
        dest="ttt_risk_gate",
        action="store_true",
        help="Enable the risk-aware pre-TTT gate based on confidence, entropy, and ensemble stability.",
    )
    parser.add_argument(
        "--no-ttt-risk-gate",
        dest="ttt_risk_gate",
        action="store_false",
        help="Disable the risk-aware pre-TTT gate and run TTT whenever --no-ttt is not set.",
    )
    parser.set_defaults(ttt_risk_gate=True)
    parser.add_argument("--ttt-risk-confidence-threshold", type=float, default=0.70)
    parser.add_argument("--ttt-risk-entropy-threshold", type=float, default=0.45)
    parser.add_argument("--ttt-risk-stability-threshold", type=float, default=0.80)
    parser.add_argument(
        "--ttt-risk-probe-n-estimators",
        type=int,
        default=8,
        help="Ensemble size used only for the baseline risk-gate probe.",
    )
    parser.add_argument(
        "--ttt-save-ckpt",
        type=parse_bool,
