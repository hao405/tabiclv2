...
class TTTConfig:
    ...
    random_state: int = 42
    ft_chunk_split_seed: Optional[int] = None
...
def parse_ft_chunk_split_seed_list(values: Optional[List[str]], *, default_seed: int) -> List[int]:
    if not values:
        return [int(default_seed)]

    seeds: List[int] = []
    for raw_value in values:
        for raw_seed in str(raw_value).split(","):
    ...
    if not seeds:
        raise ValueError("--ft-chunk-split-seed must contain at least one seed")
    if len(seeds) != len(set(seeds)):
        raise ValueError(f"--ft-chunk-split-seed contains duplicate seeds: {seeds}")
    return seeds


def resolve_seeded_out_dir(base_out_dir: Path, seed: int, *, multi_seed: bool) -> Path:
    if not multi_seed:
        return base_out_dir

    out_dir_text = str(base_out_dir)
    if "{seed}" in out_dir_text or "{qctx_seed}" in out_dir_text:
        return Path(out_dir_text.format(seed=seed, qctx_seed=seed)).expanduser()

    return base_out_dir.with_name(f"{base_out_dir.name}_seed{seed}")
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    return TTTConfig(
        ...
        ft_chunk_split_seed=(
            int(args.ft_chunk_split_seed)
            if args.ft_chunk_split_seed is not None
...
def _build_classification_meta_batch(
    ...
    *,
    ...
    split_seed = int(split_epoch_seed + chunk_idx * 7919)
    ...
    *,
    ...
    for chunk_idx, indices in enumerate(chunks):
        ...
        yield _build_classification_meta_batch(
            ...
            split_epoch_seed=split_epoch_seed,
            chunk_idx=chunk_idx,
        )
...
def build_auto_out_dir(
    ...
    *,
    ...
    raw_split_seed = getattr(args, "ft_chunk_split_seed", None)
    if isinstance(raw_split_seed, list):
        effective_split_seed = "_".join(str(seed) for seed in raw_split_seed)
    else:
        effective_split_seed = raw_split_seed if raw_split_seed is not None else args.random_state
    seed_label = f"seed{args.random_state}_qctxseed{effective_split_seed}"
    name_parts = [
        version_label,
        dataset_label,
        model_param_label,
    ...
    *,
    ...
    try:
        ...
        for epoch_idx in range(config.epochs):
            ...
            split_epoch_seed = int(config.ft_chunk_split_seed) + epoch_idx
            epoch_loss_sum = 0.0
            epoch_updates = 0
    ...
    try:
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ttt_split_reason = (
                    "TTT skipped for dataset=volkert to avoid OOM"
                    f" | ft_chunk_split_seed={ttt_config.ft_chunk_split_seed}"
                )
                classifier.fit(X_train, y_train)
            else:
                ttt_split_strategy = "full_train_epoch_chunks"
                ttt_split_reason = (
                    "full train set chunked per epoch"
                    f" | ft_chunk_split_seed={ttt_config.ft_chunk_split_seed}"
                )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir",
        default="result/compare_seed/Tabiclv2_ttt_ensemble32_data200",
        help=(
    ...
    parser.add_argument(
        "--ft-chunk-split-seed",
        "--ft-chunk-split-seeds",
        dest="ft_chunk_split_seed",
        nargs="+",
        default=None,
        help=(
            "Seed(s) used only for FT chunk-internal query/context splitting. "
            "Accepts one seed, comma-separated seeds, or space-separated seeds. "
            "Defaults to --random-state to preserve the original behavior."
        ),
    )
...
def main() -> None:
    ...
    ensure_runtime_deps()
    ft_chunk_split_seeds = parse_ft_chunk_split_seed_list(
        args.ft_chunk_split_seed,
        default_seed=int(args.random_state),
    )
    ...
    multi_seed = len(ft_chunk_split_seeds) > 1
    if multi_seed:
        print(f"ft_chunk_split_seeds: {ft_chunk_split_seeds}")

    for seed_idx, ft_chunk_split_seed in enumerate(ft_chunk_split_seeds, start=1):
        run_args = copy(args)
        run_args.ft_chunk_split_seed = int(ft_chunk_split_seed)
        out_dir = resolve_seeded_out_dir(
            base_out_dir,
            int(ft_chunk_split_seed),
        ...
        if multi_seed:
            print(
                f"[seed {seed_idx}/{len(ft_chunk_split_seeds)}] "
                f"ft_chunk_split_seed={ft_chunk_split_seed} out_dir={out_dir}",
                flush=True,
            )
        else:
            print(f"out_dir: {out_dir}")

        if run_args.models_dir is not None:
            run_multi_model_mode(run_args, dataset_dirs, gpu_ids, gpu_groups, out_dir)
        else:
            run_single_model_mode(run_args, dataset_dirs, gpu_ids, gpu_groups, out_dir)
