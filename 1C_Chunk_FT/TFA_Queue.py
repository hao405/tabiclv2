...
class ResultRow:
    ...
    ttt_task_loss: Optional[float] = None
    ttt_tfa_loss: Optional[float] = None
    ttt_tfa_z_loss: Optional[float] = None
    ttt_tfa_s_loss: Optional[float] = None
    ttt_tfa_applied: bool = False
    ttt_tfa_queue_batches: int = 0
    ttt_tfa_reference_n: int = 0
    ttt_tfa_reason: Optional[str] = None
...
class TTTConfig:
    ...
    tfa_lambda_z: float = 0.01
    tfa_lambda_s: float = 0.0
    tfa_moment: str = "mean_cov"
    tfa_z_layer: str = "row_interactor"
    tfa_s_layer: str = "icl_predictor"
    tfa_max_feature_dim: int = 256
    tfa_eps: float = 1e-6
    tfa_log_every: int = 10
    tfa_queue_enabled: bool = True
    tfa_queue_batches: int = 16
    tfa_queue_min_batches: int = 2
    tfa_queue_detach_history: bool = True
    tfa_queue_reset_each_epoch: bool = False
...
class TTTUpdateResult:
    ...
    tfa_loss: Optional[float] = None
    tfa_z_loss: Optional[float] = None
    tfa_s_loss: Optional[float] = None
    tfa_applied: bool = False
    tfa_queue_batches: int = 0
    tfa_reference_n: int = 0
    tfa_reason: Optional[str] = None
    ...
    *,
    ...
    if row.status == "ok":
        return (
            ...
            f"ft_task_loss={format_optional_float(row.ttt_task_loss)} "
            f"ft_tfa_loss={format_optional_float(row.ttt_tfa_loss)} "
            f"ft_tfa_z_loss={format_optional_float(row.ttt_tfa_z_loss)} "
            f"ft_tfa_s_loss={format_optional_float(row.ttt_tfa_s_loss)} "
            f"ft_tfa_applied={row.ttt_tfa_applied} "
            f"ft_tfa_ref_n={row.ttt_tfa_reference_n} "
            f"ft_tfa_reason={row.ttt_tfa_reason}"
        )
    if row.status == "skip":
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if float(args.tfa_lambda_z) < 0:
        raise ValueError("--tfa-lambda-z must be >= 0")
    if float(args.tfa_lambda_s) < 0:
        raise ValueError("--tfa-lambda-s must be >= 0")
    if str(args.tfa_moment) not in {"mean", "cov", "mean_cov"}:
        raise ValueError("--tfa-moment must be one of: mean, cov, mean_cov")
    if int(args.tfa_max_feature_dim) < 1:
        raise ValueError("--tfa-max-feature-dim must be >= 1")
    if float(args.tfa_eps) <= 0:
        raise ValueError("--tfa-eps must be > 0")
    if int(args.tfa_log_every) < 1:
        raise ValueError("--tfa-log-every must be >= 1")
    if int(args.tfa_queue_batches) < 1:
        raise ValueError("--tfa-queue-batches must be >= 1")
    if int(args.tfa_queue_min_batches) < 1:
        raise ValueError("--tfa-queue-min-batches must be >= 1")
    if int(args.tfa_queue_min_batches) > int(args.tfa_queue_batches):
        raise ValueError("--tfa-queue-min-batches must be <= --tfa-queue-batches")
    ...
    return TTTConfig(
        ...
        tfa_enabled=bool(args.tfa_enabled),
        tfa_lambda_z=float(args.tfa_lambda_z),
        tfa_lambda_s=float(args.tfa_lambda_s),
        tfa_moment=str(args.tfa_moment),
        tfa_z_layer=str(args.tfa_z_layer),
        tfa_s_layer=str(args.tfa_s_layer),
        tfa_max_feature_dim=int(args.tfa_max_feature_dim),
        tfa_eps=float(args.tfa_eps),
        tfa_log_every=int(args.tfa_log_every),
        tfa_queue_enabled=bool(args.tfa_queue_enabled),
        tfa_queue_batches=int(args.tfa_queue_batches),
        tfa_queue_min_batches=int(args.tfa_queue_min_batches),
        tfa_queue_detach_history=bool(args.tfa_queue_detach_history),
        tfa_queue_reset_each_epoch=bool(args.tfa_queue_reset_each_epoch),
    )
...
def _get_module_by_path(model, layer_path: str):
    module = getattr(model, "module", model)
    current = module
    for part in str(layer_path).split("."):
        part = part.strip()
        if not part:
            continue
...
def _deterministic_feature_indices(dim: int, max_dim: int, seed: int, device):
    import torch

    generator = torch.Generator(device="cpu")
    generator.manual_seed(int(seed))
    indices = torch.randperm(int(dim), generator=generator)[: int(max_dim)]
    indices, _ = torch.sort(indices)
    return indices.to(device)


def _reduce_tfa_feature(tensor, max_dim: int, seed: int):
    tensor = _first_tensor(tensor)
    if tensor is None:
        return None
    if tensor.ndim == 0:
        return None
    ...
    if feature.ndim == 1:
        ...
        feature = feature.reshape(-1, int(feature.shape[-2]), hidden_dim).mean(dim=1)
    if feature.ndim != 2 or feature.shape[0] == 0 or feature.shape[1] == 0:
        return None
    if int(feature.shape[1]) > int(max_dim):
        indices = _deterministic_feature_indices(
            int(feature.shape[1]),
            int(max_dim),
            int(seed),
            feature.device,
...
def _compute_feature_stats(features, eps: float):
    import torch

    if features is None or features.ndim != 2 or features.shape[0] == 0:
        return None
    feats = features.float()
    mean = feats.mean(dim=0).detach()
    cov = None
    if int(feats.shape[0]) >= 2:
        centered = feats - feats.mean(dim=0, keepdim=True)
        cov = (centered.transpose(0, 1) @ centered) / max(1, int(feats.shape[0]) - 1)
        eye = torch.eye(cov.shape[0], device=cov.device, dtype=cov.dtype)
        cov = (cov + float(eps) * eye).detach()
    return {"mean": mean, "cov": cov, "n": int(feats.shape[0])}


def _compute_moment_loss(current_feats, ref_stats: Optional[dict], moment: str, eps: float):
    import torch

    if ref_stats is None or current_feats is None or current_feats.ndim != 2 or current_feats.shape[0] == 0:
        zero = torch.tensor(0.0)
        return zero, zero, zero
    zero = current_feats.float().sum() * 0.0
    feats = current_feats.float()
    ref_mean = ref_stats.get("mean")
    if ref_mean is None:
        return zero, zero, zero
    ref_mean = ref_mean.to(device=feats.device, dtype=feats.dtype).detach()
    mean_loss = ((feats.mean(dim=0) - ref_mean) ** 2).mean()
    cov_loss = zero
    if moment in {"cov", "mean_cov"} and int(feats.shape[0]) >= 2 and ref_stats.get("cov") is not None:
        centered = feats - feats.mean(dim=0, keepdim=True)
        cov = (centered.transpose(0, 1) @ centered) / max(1, int(feats.shape[0]) - 1)
        eye = torch.eye(cov.shape[0], device=cov.device, dtype=cov.dtype)
        cov = cov + float(eps) * eye
        ref_cov = ref_stats["cov"].to(device=feats.device, dtype=feats.dtype).detach()
        if cov.shape == ref_cov.shape:
            cov_loss = ((cov - ref_cov) ** 2).mean()
    if moment == "mean":
        total = mean_loss
    elif moment == "cov":
        total = cov_loss
    else:
        total = mean_loss + cov_loss
    return total, mean_loss, cov_loss


class FeatureMomentQueue:
    def __init__(
        self,
        *,
        max_batches: int,
        ...
        self._features = deque(maxlen=self.max_batches)
    ...
    def get_features(self, current_feature):
        if current_feature is None:
            return None
        if len(self._features) + 1 < self.min_batches:
            return current_feature
        history = [
            feature.to(device=current_feature.device, dtype=current_feature.dtype)
        ...
        return torch.cat(history + [current_feature], dim=0)

    def append(self, current_feature) -> None:
        if current_feature is None:
            return
        history_feature = current_feature.float()
        if self.detach_history:
            history_feature = history_feature.detach()
        self._features.append(history_feature)


class TFAFeatureCollector:
    def __init__(
        self,
        model,
        ...
        *,
        ...
        self.layer_paths = dict(layer_paths)
    ...
    def register(self) -> None:
        for key, layer_path in self.layer_paths.items():
            module = _get_module_by_path(self.model, layer_path)
            if module is None:
                reason = f"layer_not_found:{layer_path}"
                self.disabled_reasons[key] = reason
                print(
                    f"[tfa-warning] model={self.model_name} dataset={self.dataset_name} "
                    f"{key}_layer={layer_path!r} not found; disabling {key}-feature alignment",
                    file=sys.stderr,
                    flush=True,
                )
                continue
            self.active_layers[key] = layer_path
            self.handles.append(
    ...
    def _make_hook(self, key: str):
        def _hook(_module, _inputs, output):
            feature = _reduce_tfa_feature(output, self.max_dim, self.seed + (17 if key == "s" else 0))
            if feature is not None:
                self.features.setdefault(key, []).append(feature)
    ...
    def pop(self, target_device=None) -> Dict[str, Any]:
        import torch
        ...
        for key, values in self.features.items():
            ...
            if len(values) == 1:
                ...
                if target_device is not None:
                    feature = feature.to(target_device)
                captured[key] = feature
                continue
            device = target_device if target_device is not None else values[0].device
            captured[key] = torch.cat([feature.to(device) for feature in values], dim=0)
        self.clear()
        return captured
...
def _append_tfa_reason(existing: Optional[str], addition: Optional[str]) -> Optional[str]:
    if not addition:
        return existing
    if not existing:
        return addition
...
def _collect_tfa_reference_stats(
    classifier,
    forward_model,
    X_encoded,
    y_encoded,
    config: TTTConfig,
    collector: TFAFeatureCollector,
    *,
    device,
    model_name: str,
    dataset_name: str,
) -> tuple[Dict[str, dict], int, Optional[str]]:
    if not config.tfa_enabled:
        return {}, 0, "disabled"
    if not collector.active_layers:
        reason = "no_active_tfa_layers"
        if collector.disabled_reasons:
            reason += ":" + ",".join(
                f"{key}={value}" for key, value in sorted(collector.disabled_reasons.items())
            )
        return {}, 0, reason

    import torch

    ref_features: Dict[str, List[Any]] = {key: [] for key in collector.active_layers}
    previous_training = forward_model.training
    try:
        ...
        with torch.inference_mode():
            for batch in iter_epoch_meta_batches(
                ...
                y_encoded,
                config=config,
                epoch_seed=config.random_state,
            ):
                if batch.skip_reason:
                    continue
                batch = move_meta_batch(batch, device)
                collector.clear()
                _ = forward_model(batch.X, batch.y_train.float())
                captured = collector.pop(target_device=device)
                for key, feature in captured.items():
                    if key in ref_features and feature is not None:
                        ref_features[key].append(feature.detach().float())
        ref_stats: Dict[str, dict] = {}
        for key, features in ref_features.items():
            if not features:
                continue
            cat = torch.cat(features, dim=0)
            stats = _compute_feature_stats(cat, config.tfa_eps)
            if stats is not None:
                ref_stats[key] = stats
        reference_n = 0
        if "z" in ref_stats:
            reference_n = int(ref_stats["z"].get("n", 0))
        elif "s" in ref_stats:
            reference_n = int(ref_stats["s"].get("n", 0))
        if not ref_stats:
            return {}, 0, "no_valid_reference_features"
        print(
            f"[tfa-ref] model={model_name} dataset={dataset_name} "
            f"layers={','.join(sorted(ref_stats.keys()))} reference_n={reference_n}",
            flush=True,
        )
        return ref_stats, reference_n, None
    except Exception as exc:
        return {}, 0, f"reference_failed:{type(exc).__name__}:{exc}"
    finally:
        collector.clear()
...
def build_auto_out_dir(
    ...
    *,
    ...
    if args.ttt_enabled:
        ...
        if args.tfa_enabled:
            queue_label = f"q{args.tfa_queue_batches}" if args.tfa_queue_enabled else "noq"
            eval_estimator_parts.append(
                f"tfa_z{args.tfa_lambda_z}_s{args.tfa_lambda_s}_{args.tfa_moment}_{queue_label}"
            )
        else:
            eval_estimator_parts.append("no_tfa")
        eval_estimator_label = "_".join(eval_estimator_parts)
    else:
...
def run_ttt_epoch_chunk_update(
    ...
    *,
    ...
    tfa_reference_n = 0
    tfa_reason: Optional[str] = None
    tfa_ref_stats: Dict[str, dict] = {}
    tfa_queues: Dict[str, FeatureMomentQueue] = {}
    tfa_collector: Optional[TFAFeatureCollector] = None
    ...
    try:
        if X_encoded.shape[0] < 2 or chunks_per_epoch == 0:
            return TTTUpdateResult(
                ...
                tfa_applied=False,
                tfa_queue_batches=(config.tfa_queue_batches if config.tfa_queue_enabled else 0),
                tfa_reference_n=tfa_reference_n,
                tfa_reason="not_initialized",
            )

        if config.tfa_enabled and (config.tfa_lambda_z > 0 or config.tfa_lambda_s > 0):
            tfa_collector = TFAFeatureCollector(
                base_model,
                {
                    "z": config.tfa_z_layer,
                    "s": config.tfa_s_layer,
                },
                max_dim=config.tfa_max_feature_dim,
                seed=config.random_state,
                model_name=model_name,
                dataset_name=dataset_name,
            )
            tfa_collector.register()
            if tfa_collector.disabled_reasons:
                disabled_reason = ",".join(
                    f"{key}={value}" for key, value in sorted(tfa_collector.disabled_reasons.items())
                )
                tfa_reason = _append_tfa_reason(tfa_reason, f"disabled_layers:{disabled_reason}")
            tfa_ref_stats, tfa_reference_n, ref_reason = _collect_tfa_reference_stats(
                classifier,
                forward_model,
                X_encoded,
                y_encoded,
            ...
            if ref_reason is not None:
                tfa_reason = _append_tfa_reason(tfa_reason, ref_reason)
                tfa_ref_stats = {}
                print(
                    f"[tfa-warning] model={model_name} dataset={dataset_name} "
                    f"TFA disabled: {ref_reason}",
                    file=sys.stderr,
                    flush=True,
                )
            if config.tfa_lambda_z > 0 and "z" not in tfa_ref_stats:
                tfa_reason = _append_tfa_reason(tfa_reason, "z_reference_unavailable")
            if config.tfa_lambda_s > 0 and "s" not in tfa_ref_stats:
                tfa_reason = _append_tfa_reason(tfa_reason, "s_reference_unavailable")
            if config.tfa_lambda_z > 0 and "z" in tfa_ref_stats:
                tfa_queues["z"] = FeatureMomentQueue(
                    max_batches=config.tfa_queue_batches,
                    min_batches=config.tfa_queue_min_batches,
                    detach_history=config.tfa_queue_detach_history,
                )
            if config.tfa_lambda_s > 0 and "s" in tfa_ref_stats:
                tfa_queues["s"] = FeatureMomentQueue(
                    max_batches=config.tfa_queue_batches,
                    min_batches=config.tfa_queue_min_batches,
                    detach_history=config.tfa_queue_detach_history,
                )
        elif config.tfa_enabled:
            tfa_reason = "all_tfa_lambdas_zero"
        else:
            tfa_reason = "disabled"
        ...
        for epoch_idx in range(config.epochs):
            _set_ttt_train_mode(classifier)
            if config.tfa_queue_reset_each_epoch:
                for feature_queue in tfa_queues.values():
                    feature_queue.reset()
            epoch_seed = config.random_state + epoch_idx
            epoch_loss_sum = 0.0
            epoch_task_loss_sum = 0.0
            epoch_tfa_loss_sum = 0.0
            epoch_updates = 0
            for batch in iter_epoch_meta_batches(
                classifier,
                X_encoded,
                ...
                batch_z_loss = 0.0
                batch_s_loss = 0.0
                batch_z_mean_loss = 0.0
                batch_z_cov_loss = 0.0
                batch_s_mean_loss = 0.0
                batch_s_cov_loss = 0.0
                ...
                for start_idx in range(0, total_views, effective_micro_batch_size):
                    ...
                    with amp_ctx_factory():
                        ...
                        task_loss = F.cross_entropy(logits_used, y_query_batch.long().reshape(-1))
                        loss = task_loss
                        tfa_loss = task_loss * 0.0
                        z_total = task_loss * 0.0
                        z_mean = task_loss * 0.0
                        z_cov = task_loss * 0.0
                        s_total = task_loss * 0.0
                        s_mean = task_loss * 0.0
                        s_cov = task_loss * 0.0
                        captured_features = {}
                        if tfa_collector is not None and tfa_ref_stats:
                            try:
                                captured_features = tfa_collector.pop(target_device=task_loss.device)
                                z_feature = captured_features.get("z")
                                if config.tfa_lambda_z > 0 and z_feature is not None and "z" in tfa_ref_stats:
                                    z_moment_feature = z_feature
                                    if config.tfa_queue_enabled and "z" in tfa_queues:
                                        z_moment_feature = tfa_queues["z"].get_features(z_feature)
                                    z_total, z_mean, z_cov = _compute_moment_loss(
                                        z_moment_feature,
                                        tfa_ref_stats["z"],
                                        config.tfa_moment,
                                        config.tfa_eps,
                                    )
                                    tfa_loss = tfa_loss + float(config.tfa_lambda_z) * z_total
                                s_feature = captured_features.get("s")
                                if config.tfa_lambda_s > 0 and s_feature is not None and "s" in tfa_ref_stats:
                                    s_moment_feature = s_feature
                                    if config.tfa_queue_enabled and "s" in tfa_queues:
                                        s_moment_feature = tfa_queues["s"].get_features(s_feature)
                                    s_total, s_mean, s_cov = _compute_moment_loss(
                                        s_moment_feature,
                                        tfa_ref_stats["s"],
                                        config.tfa_moment,
                                        config.tfa_eps,
                                    )
                                    tfa_loss = tfa_loss + float(config.tfa_lambda_s) * s_total
                                if tfa_loss.detach().abs().item() > 0.0:
                                    tfa_applied = True
                                loss = task_loss + tfa_loss
                            except Exception as exc:
                                tfa_reason = _append_tfa_reason(
                                    tfa_reason,
                                    f"train_failed:{type(exc).__name__}:{exc}",
                                )
                                tfa_ref_stats = {}
                                tfa_queues.clear()
                                print(
                                    f"[tfa-warning] model={model_name} dataset={dataset_name} "
                                    f"TFA disabled during training: {type(exc).__name__}: {exc}",
                                    file=sys.stderr,
                                    flush=True,
                                )
                                loss = task_loss
                    ...
                    weight = float(X_batch.shape[0]) / float(total_views)
                    task_value = float((task_loss.detach() * weight).cpu())
                    tfa_value = float((tfa_loss.detach() * weight).cpu())
                    z_value = float((z_total.detach() * weight).cpu())
                    s_value = float((s_total.detach() * weight).cpu())
                    z_mean_value = float((z_mean.detach() * weight).cpu())
                    z_cov_value = float((z_cov.detach() * weight).cpu())
                    s_mean_value = float((s_mean.detach() * weight).cpu())
                    s_cov_value = float((s_cov.detach() * weight).cpu())
                    batch_task_loss += task_value
                    batch_tfa_loss += tfa_value
                    batch_z_loss += z_value
                    batch_s_loss += s_value
                    batch_z_mean_loss += z_mean_value
                    batch_z_cov_loss += z_cov_value
                    batch_s_mean_loss += s_mean_value
                    batch_s_cov_loss += s_cov_value
                    if tfa_collector is not None and tfa_ref_stats and captured_features:
                        if config.tfa_queue_enabled:
                            z_history = captured_features.get("z")
                            if z_history is not None and "z" in tfa_queues:
                                tfa_queues["z"].append(z_history)
                            s_history = captured_features.get("s")
                            if s_history is not None and "s" in tfa_queues:
                                tfa_queues["s"].append(s_history)
                ...
                epoch_task_loss_sum += batch_task_loss
                epoch_tfa_loss_sum += batch_tfa_loss
                last_loss = batch_loss
                task_loss_sum += batch_task_loss
                tfa_loss_sum += batch_tfa_loss
                tfa_z_loss_sum += batch_z_loss
                tfa_s_loss_sum += batch_s_loss
                task_loss_count += 1
                tfa_loss_count += 1
                current_lr = scheduler.get_last_lr()[0]

                if update_steps % 3 == 0 or (tfa_ref_stats and update_steps % config.tfa_log_every == 0):
                    print(
                        f"[ft-loss] model={model_name} dataset={dataset_name} "
                        f"epoch={epoch_idx + 1}/{config.epochs} step={update_steps} "
                        f"loss={batch_loss:.6f} "
                        f"task_loss={batch_task_loss:.6f} "
                        f"tfa_loss={batch_tfa_loss:.6f} "
                        f"z_loss={batch_z_loss:.6f} "
                        f"z_mean_loss={batch_z_mean_loss:.6f} "
                        f"z_cov_loss={batch_z_cov_loss:.6f} "
                        f"s_loss={batch_s_loss:.6f} "
                        f"s_mean_loss={batch_s_mean_loss:.6f} "
                        f"s_cov_loss={batch_s_cov_loss:.6f} "
                        f"lr={current_lr:.2e}",
                        flush=True,
                    )
            ...
            if epoch_updates > 0:
                print(
                    ...
                    f"mean_task_loss={epoch_task_loss_sum / epoch_updates:.6f} "
                    f"mean_tfa_loss={epoch_tfa_loss_sum / epoch_updates:.6f} "
        ...
        if update_steps == 0:
            ...
            return TTTUpdateResult(
                ...
                task_loss=_mean_or_none(task_loss_sum, task_loss_count),
                tfa_loss=_mean_or_none(tfa_loss_sum, tfa_loss_count),
                tfa_z_loss=_mean_or_none(tfa_z_loss_sum, tfa_loss_count),
                tfa_s_loss=_mean_or_none(tfa_s_loss_sum, tfa_loss_count),
                tfa_applied=tfa_applied,
                tfa_queue_batches=(config.tfa_queue_batches if config.tfa_queue_enabled else 0),
                tfa_reference_n=tfa_reference_n,
                tfa_reason=tfa_reason,
            )
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ...
        ttt_tfa_z_loss = None
        ttt_tfa_s_loss = None
        ttt_tfa_applied = False
        ttt_tfa_queue_batches = 0
        ttt_tfa_reference_n = 0
        ttt_tfa_reason = None
        n_train_b = 0
        n_holdout_c = 0
        ...
        if ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                try:
                    ...
                    ttt_task_loss = ttt_result.task_loss
                    ttt_tfa_loss = ttt_result.tfa_loss
                    ttt_tfa_z_loss = ttt_result.tfa_z_loss
                    ttt_tfa_s_loss = ttt_result.tfa_s_loss
                    ttt_tfa_applied = ttt_result.tfa_applied
                    ttt_tfa_queue_batches = ttt_result.tfa_queue_batches
                    ttt_tfa_reference_n = ttt_result.tfa_reference_n
                    ttt_tfa_reason = ttt_result.tfa_reason
                    if ttt_result.reason:
                        ttt_split_reason = append_ttt_reason(ttt_split_reason, ttt_result.reason)
                    if ttt_tfa_reason:
                        ttt_split_reason = append_ttt_reason(ttt_split_reason, f"tfa={ttt_tfa_reason}")
        ...
        return ResultRow(
            ...
            ttt_task_loss=ttt_task_loss,
            ttt_tfa_loss=ttt_tfa_loss,
            ttt_tfa_z_loss=ttt_tfa_z_loss,
            ttt_tfa_s_loss=ttt_tfa_s_loss,
            ttt_tfa_applied=ttt_tfa_applied,
            ttt_tfa_queue_batches=ttt_tfa_queue_batches,
            ttt_tfa_reference_n=ttt_tfa_reference_n,
            ttt_tfa_reason=ttt_tfa_reason,
        )
    except Exception as exc:
        return ResultRow(
            dataset_name=dataset_dir.name,
...
def write_summary(
    ...
    tfa_applied_df = (
        ok_df[truthy_column_mask(ok_df, "ttt_tfa_applied")].copy()
        if len(ok_df)
        else pd.DataFrame()
    )
    ...
    lines = [
        ...
        f"ft_tfa_applied_count: {len(tfa_applied_df)}",
        ...
        mean_line("avg_ft_task_loss_ok", "ttt_task_loss"),
        mean_line("avg_ft_tfa_loss_ok", "ttt_tfa_loss"),
        mean_line("avg_ft_tfa_z_loss_ok", "ttt_tfa_z_loss"),
        mean_line("avg_ft_tfa_s_loss_ok", "ttt_tfa_s_loss"),
    ...
    if len(tfa_applied_df):
        tfa_applied_names = ", ".join(tfa_applied_df["dataset_name"].astype(str).tolist())
        lines.append(f"ft_tfa_applied_datasets: {tfa_applied_names}")
    else:
        lines.append("ft_tfa_applied_datasets: (none)")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--out-dir",
        default="results/TTT+/Tabiclv2_FT+TTT_data200",
        help=(
    ...
    parser.set_defaults(exclude_ttt_inactive_datasets=True)
    ...
    parser.add_argument("--tfa-enabled", type=parse_bool, default=True)
    parser.add_argument("--tfa-lambda-z", type=float, default=0.01)
    parser.add_argument("--tfa-lambda-s", type=float, default=0.0)
    parser.add_argument("--tfa-moment", choices=["mean", "cov", "mean_cov"], default="mean_cov")
    parser.add_argument("--tfa-z-layer", default="row_interactor")
    parser.add_argument("--tfa-s-layer", default="icl_predictor")
    parser.add_argument("--tfa-max-feature-dim", type=int, default=256)
    parser.add_argument("--tfa-eps", type=float, default=1e-6)
    parser.add_argument("--tfa-log-every", type=int, default=10)
    parser.add_argument("--tfa-queue-enabled", type=parse_bool, default=True)
    parser.add_argument("--tfa-queue-batches", type=int, default=16)
    parser.add_argument("--tfa-queue-min-batches", type=int, default=2)
    parser.add_argument("--tfa-queue-detach-history", type=parse_bool, default=True)
    parser.add_argument("--tfa-queue-reset-each-epoch", type=parse_bool, default=False)
    parser.add_argument(
        "--ttt-data-parallel",
...
def resolve_gpu_assignments(args: argparse.Namespace) -> tuple[List[int], List[str]]:
    if args.gpu_groups:
        ...
        if len(gpu_groups) > args.workers:
            gpu_groups = gpu_groups[: args.workers]
        if len(gpu_groups) != args.workers:
