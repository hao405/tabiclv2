...
WISE_SMALL_DATASET_MAX_ROWS = 2000
WISE_LARGE_DATASET_MIN_ROWS = 8000
WISE_MID_DATASET_ALPHA = 1.0
WISE_ALPHA_MIN = 0.0
WISE_ALPHA_MAX = 2.0
...
class TTTConfig:
    ...
    wise_ft: bool = True
    wise_alpha: float = 0.5
    wise_large_alpha: float = 1.1
    epochs: int = 30
    steps: int = 30
...
def build_ttt_config(args: argparse.Namespace) -> TTTConfig:
    ...
    if not WISE_ALPHA_MIN <= float(args.ttt_wise_alpha) <= WISE_ALPHA_MAX:
        raise ValueError(f"--ttt-wise-alpha must be in [{WISE_ALPHA_MIN:g}, {WISE_ALPHA_MAX:g}]")
    if not WISE_ALPHA_MIN <= float(args.ttt_wise_large_alpha) <= WISE_ALPHA_MAX:
        raise ValueError(f"--ttt-wise-large-alpha must be in [{WISE_ALPHA_MIN:g}, {WISE_ALPHA_MAX:g}]")
    ...
    return TTTConfig(
        ...
        wise_large_alpha=float(args.ttt_wise_large_alpha),
    ...
    if args.ttt_enabled:
        eval_estimator_label = "_".join(
            [
                ...
                f"wiselargealpha{_format_path_value(args.ttt_wise_large_alpha)}",
            ]
        )
    else:
    ...
    *,
    ...
    if config.epochs < 1:
        return TTTUpdateResult(
            ...
            wise_alpha=float(config.wise_alpha) if config.wise_ft else None,
    ...
    base_model = _get_ttt_base_model(classifier)
    initial_state = _clone_model_state_dict_cpu(base_model) if config.wise_ft else None
    ...
    try:
        ...
        if config.wise_ft and initial_state is not None:
            _apply_wise_ft_weights(base_model, initial_state, config.wise_alpha)
            wise_applied = True
            print(
                f"[ft-wise] model={model_name} dataset={dataset_name} "
...
def evaluate_one_dataset(
    ...
    *,
    ...
    try:
        ...
        apply_large_wise_alpha = bool(ttt_config.wise_ft) and dataset_total_rows > WISE_LARGE_DATASET_MIN_ROWS
        apply_mid_wise_alpha = (
            bool(ttt_config.wise_ft)
            and WISE_SMALL_DATASET_MAX_ROWS <= dataset_total_rows <= WISE_LARGE_DATASET_MIN_ROWS
        )
        dataset_ttt_config = (
            replace(ttt_config, wise_alpha=float(ttt_config.wise_large_alpha))
            if apply_large_wise_alpha
            else replace(ttt_config, wise_alpha=WISE_MID_DATASET_ALPHA)
            if apply_mid_wise_alpha
            else ttt_config
        )

        ttt_loss = None
        ttt_wise_ft = bool(dataset_ttt_config.wise_ft) if dataset_ttt_config.enabled else False
        ttt_wise_alpha = (
            float(dataset_ttt_config.wise_alpha)
            if dataset_ttt_config.enabled and dataset_ttt_config.wise_ft
            else None
        )
        ttt_wise_applied = False
        ttt_steps = 0
        ttt_lr = dataset_ttt_config.lr if dataset_ttt_config.enabled else None
        ttt_applied = False
        ...
        if dataset_ttt_config.enabled:
            if should_skip_ttt_for_dataset(dataset_dir, info):
                ...
                if dataset_ttt_config.wise_ft:
                    if apply_large_wise_alpha:
                        ttt_split_reason += (
                            f" | wise_ft=large_alpha{dataset_ttt_config.wise_alpha:g}"
                            f" rows={dataset_total_rows}>{WISE_LARGE_DATASET_MIN_ROWS}"
                        )
                    elif dataset_total_rows < WISE_SMALL_DATASET_MAX_ROWS:
                        ttt_split_reason += (
                            f" | wise_ft=alpha{dataset_ttt_config.wise_alpha:g}"
                            f" rows={dataset_total_rows}<{WISE_SMALL_DATASET_MAX_ROWS}"
                        )
                    else:
                        ttt_split_reason += (
                            f" | wise_ft=mid_alpha{dataset_ttt_config.wise_alpha:g}"
                            f" rows={dataset_total_rows} in "
                            f"[{WISE_SMALL_DATASET_MAX_ROWS},{WISE_LARGE_DATASET_MIN_ROWS}]"
                        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "epoch-shuffled chunk FT, WiSE-FT interpolation, and AMD/ROCm multi-GPU workers."
        )
    )
    ...
    parser.add_argument(
        "--out-dir",
        default="results/wise_ft/Tabiclv2_wise_ft_ensemble32_small_0.1_large_1.1",
        help=(
            "Output directory. If omitted, generate one under 1b_result from "
            "TabICL version, dataset label, model parameters, FT/WiSE-FT eval metric, "
    ...
    parser.add_argument(
        ...
        type=parse_bool,
        default=True,
        help="Apply WiSE-FT weight interpolation after the FT update path.",
    )
    parser.add_argument(
        "--ttt-wise-alpha",
        type=float,
        default=0.1,
        help=(
            "WiSE-FT interpolation alpha for datasets with total rows < 2000. "
            "Datasets with total rows from 2000 to 8000 use fixed alpha=1.0. "
            "0 keeps the initial model weights, 1 keeps the fine-tuned endpoint, "
            "and values above 1 extrapolate past the fine-tuned endpoint."
        ),
    )
    parser.add_argument(
        "--ttt-wise-large-alpha",
        type=float,
        default=1.1,
        help=(
            "WiSE-FT interpolation alpha for datasets with total rows > 8000. "
            "Defaults to 1.1 to extrapolate from the initial weights past the "
            "fine-tuned endpoint on large datasets."
        ),
    )
    parser.add_argument(
        "--ttt-epochs",
