...
V2_CLASSIFIER_FILE = "tabpfn-v2-classifier-v2_default.ckpt"
...
class TabPFNAdapter:
    ...
    def _make_classifier(self, loaded: LoadedDataset):
        try:
        ...
        classifier_kwargs = self._classifier_kwargs(loaded.categorical_feature_indices)
        ...
        try:
            ...
            if hasattr(classifier, "n_estimators"):
                classifier.n_estimators = self.args.n_estimators
    ...
    @staticmethod
    def _classes_for_probabilities(classifier: Any, y_proba: Any) -> np.ndarray | None:
        ...
        candidates = [
            ...
            getattr(getattr(classifier, "estimator", None), "classes_", None),
        ]
        for candidate in candidates:
            if candidate is None:
                continue
            classes = np.asarray(candidate)
            if classes.ndim == 1 and len(classes) == proba_arr.shape[1]:
                return classes
        return None
    ...
    def _predict_with_probabilities(
        ...
        try:
            y_proba = np.asarray(classifier.predict_proba(X_test))
            classes = self._classes_for_probabilities(classifier, y_proba)
    ...
    def fit_predict(self, loaded: LoadedDataset) -> PredictionResult:
        ...
        y_pred, y_proba, classes = self._predict_with_probabilities(classifier, X_test)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TabPFN on data178 classification datasets.",
    ...
    parser.add_argument("--many-class-n-estimators", type=int, default=8)
    parser.add_argument(
